...
?>
<?php if (!I18n::factory('index')->object()->count()): ?>
...
<div class="row">
    <form class="form-search fl_l">
        <div class="input-prepend">
            ...
            <input type="text" id="i18n-search" name="q" class="input-xxlarge" placeholder="<?php echo t("Введите строку для поиска…") ?>" value="<?php echo cogear()->input->get('q', ''); ?>">
        </div>
        <button type="submit" class="btn"><?php echo t('Найти') ?></button>
    </form>
    <button class="btn btn-mini fl_r" id="i18n-reset" style="margin-top: 3px;"><?php echo t("Сбросить") ?></button>
    <button class="btn btn-primary fl_r" style="margin-right: 5px;" id="i18n-action-button" data-action="<?php echo $base . $path ?>"><?php echo t("Фильтровать") ?></button>
</div>
<script>
    $('#i18n-search').on('keyup',function(){
        $this = $(this);
        if($this.val()){
...
<script>
    ...
    $(document).ready(function(){
        ...
        $('#i18n-reset').on('click',function(event){
            event.preventDefault();
            $(this).next().attr('data-action','<?php echo l('/admin/lang/translate/') ?>')
            $(this).next().click();
            return false;
        })
        $('#i18n-action-button').on('click',function(event){
