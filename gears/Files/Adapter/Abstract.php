 ...
 **/
abstract class Files_Adapter_Abstract {
	...
	public function mount(array $opts) {
		...
		$this->id = $this->driverId.(!empty($this->options['id']) ? $this->options['id'] : Files_Handler::$volumesCnt++).'_';
	...
	public function file($hash) {
		...
		return ($file = $this->stat($path)) ? $file : $this->setError(Files_Handler::ERROR_FILE_NOT_FOUND);
		...
		return $this->setError(Files_Handler::ERROR_FILE_NOT_FOUND);
	}
	...
	public function dir($hash, $resolveLink=false) {
		if (($dir = $this->file($hash)) == false) {
			return $this->setError(Files_Handler::ERROR_DIR_NOT_FOUND);
		}
		...
		return $dir && $dir['mime'] == 'directory' && empty($dir['hidden'])
			? $dir
			: $this->setError(Files_Handler::ERROR_NOT_DIR);
	}
	...
	public function scandir($hash) {
		...
		return $dir['read']
			...
			: $this->setError(Files_Handler::ERROR_PERM_DENIED);
	}
	...
	public function mkdir($dst, $name) {
		if ($this->commandDisabled('mkdir')) {
			return $this->setError(Files_Handler::ERROR_PERM_DENIED);
		}

		if (!$this->nameAccepted($name)) {
			return $this->setError(Files_Handler::ERROR_INVALID_NAME);
		}

		if (($dir = $this->dir($dst)) == false) {
			return $this->setError(Files_Handler::ERROR_TRGDIR_NOT_FOUND, '#'.$dst);
		}
		...
		$stat = $this->stat($dst);
		if (!empty($stat)) {
			return $this->setError(Files_Handler::ERROR_EXISTS, $name);
		}
		$this->clearcache();
	...
	public function rename($hash, $name) {
		...
		if (!$this->nameAccepted($name)) {
			return $this->setError(Files_Handler::ERROR_INVALID_NAME, $name);
		}
		...
		if (!empty($file['locked'])) {
			return $this->setError(Files_Handler::ERROR_LOCKED, $file['name']);
		}
	...
	public function duplicate($hash, $suffix='copy') {
		if ($this->commandDisabled('duplicate')) {
			return $this->setError(Files_Handler::ERROR_COPY, '#'.$hash, Files_Handler::ERROR_PERM_DENIED);
		}

		if (($file = $this->file($hash)) == false) {
			return $this->setError(Files_Handler::ERROR_COPY, Files_Handler::ERROR_FILE_NOT_FOUND);
		}
	...
	public function upload($fp, $dst, $name, $tmpname) {
		...
		if ($mime == 'unknown' && $this->mimeDetect == 'internal') {
			$mime = Files_Adapter_Abstract::mimetypeInternalDetect($name);
		}
		...
		if (!$upload) {
			return $this->setError(Files_Handler::ERROR_UPLOAD_FILE_MIME);
		}

		if ($this->uploadMaxSize > 0 && filesize($tmpname) > $this->uploadMaxSize) {
			return $this->setError(Files_Handler::ERROR_UPLOAD_FILE_SIZE);
		}
		...
		if ($file) { // file exists
			if ($this->options['uploadOverwrite']) {
				if (!$file['write']) {
					...
					return $this->setError(Files_Handler::ERROR_NOT_REPLACE, $name);
				}
				$this->remove($file);
			} else {
	...
	public function paste($volume, $src, $dst, $rmSrc = false) {
		$err = $rmSrc ? Files_Handler::ERROR_MOVE : Files_Handler::ERROR_COPY;

		if ($this->commandDisabled('paste')) {
			return $this->setError($err, '#'.$src, Files_Handler::ERROR_PERM_DENIED);
		}

		if (($file = $volume->file($src, $rmSrc)) == false) {
			return $this->setError($err, '#'.$src, Files_Handler::ERROR_FILE_NOT_FOUND);
		}
		...
		if (($dir = $this->dir($dst)) == false) {
			return $this->setError($err, $errpath, Files_Handler::ERROR_TRGDIR_NOT_FOUND, '#'.$dst);
		}

		if (!$dir['write'] || !$file['read']) {
			return $this->setError($err, $errpath, Files_Handler::ERROR_PERM_DENIED);
		}
		...
		if (($test = $volume->closest($src, $rmSrc ? 'locked' : 'read', $rmSrc))) {
			return $rmSrc
				? $this->setError($err, $errpath, Files_Handler::ERROR_LOCKED, $volume->path($test))
				: $this->setError($err, $errpath, Files_Handler::ERROR_PERM_DENIED);
		}
		...
		if ($stat) {
			if ($this->options['copyOverwrite']) {
				...
				if (!$this->isSameType($file['mime'], $stat['mime'])) {
					return $this->setError(Files_Handler::ERROR_NOT_REPLACE, $this->_path($test));
				}
				...
				if (($locked = $this->closestByAttr($test, 'locked', true))) {
					return $this->setError(Files_Handler::ERROR_LOCKED, $this->_path($locked));
				}
				// remove existed file
				if (!$this->remove($test)) {
					return $this->setError(Files_Handler::ERROR_REPLACE, $this->_path($test));
				}
			} else {
		...
		if ($volume == $this) {
			...
			if ($this->_inpath($destination, $source)) {
				return $this->setError(Files_Handler::ERROR_COPY_INTO_ITSELF, $path);
			}
		...
		if (!$this->options['copyTo'] || !$volume->copyFromAllowed()) {
			return $this->setError(Files_Handler::ERROR_COPY, $errpath, Files_Handler::ERROR_PERM_DENIED);
		}
		...
		if ($rmSrc) {
			if ($volume->rm($src)) {
				...
				return $this->setError(Files_Handler::ERROR_MOVE, $errpath, Files_Handler::ERROR_RM_SRC);
			}
		}
		return $this->stat($path);
	}
	...
	public function getContents($hash) {
		...
		if ($file['mime'] == 'directory') {
			return $this->setError(Files_Handler::ERROR_NOT_FILE);
		}
	...
	public function extract($hash) {
		...
		if (!$archiver) {
			return $this->setError(Files_Handler::ERROR_NOT_ARCHIVE);
		}
	...
	public function archive($hashes, $mime) {
		...
		if (!$archiver) {
			return $this->setError(Files_Handler::ERROR_ARCHIVE_TYPE);
		}
		...
		foreach ($hashes as $hash) {
			if (($file = $this->file($hash)) == false) {
				return $this->error(Files_Handler::ERROR_FILE_NOT_FOUND, '#'+$hash);
			}
			if (!$file['read']) {
				return $this->error(Files_Handler::ERROR_PERM_DENIED);
			}
			$path = $this->decode($hash);
			if (!isset($dir)) {
				$dir = $this->_dirname($path);
	...
	public function resize($hash, $width, $height, $x, $y, $mode = 'resize', $bg = '', $degree = 0) {
		...
		if (!$this->canResize($path, $file)) {
			return $this->setError(Files_Handler::ERROR_UNSUPPORT_TYPE);
		}
	...
	public function rm($hash) {
		return $this->commandDisabled('rm')
			? array(Files_Handler::ERROR_ACCESS_DENIED)
	...
	protected function mimetype($path) {
		...
		if ($this->mimeDetect == 'finfo') {
			...
			$type = Files_Adapter_Abstract::mimetypeInternalDetect($path);
		}
		...
		return $type == 'unknown' && $this->mimeDetect != 'internal'
			? Files_Adapter_Abstract::mimetypeInternalDetect($path)
			: $type;
	...
	static protected function mimetypeInternalDetect($path) {
		...
		return isset(Files_Adapter_Abstract::$mimetypes[$ext]) ? Files_Adapter_Abstract::$mimetypes[$ext] : 'unknown';
	...
	protected function copy($src, $dst, $name) {
		...
		if (!empty($srcStat['thash'])) {
			...
			return $stat && $this->_symlink($target, $dst, $name)
				...
				: $this->setError(Files_Handler::ERROR_COPY, $this->_path($src));
		}
		...
		if ($srcStat['mime'] == 'directory') {
			...
			if (($test && $test['mime'] != 'directory') || !$this->_mkdir($dst, $name)) {
				return $this->setError(Files_Handler::ERROR_COPY, $this->_path($src));
			}
	...
	protected function move($src, $dst, $name) {
		...
		return $this->setError(Files_Handler::ERROR_MOVE, $this->_path($src));
	}
	...
	protected function copyFrom($volume, $src, $destination, $name) {
		...
		if (($source = $volume->file($src)) == false) {
			return $this->setError(Files_Handler::ERROR_COPY, '#'.$src, $volume->error());
		}

		$errpath = $volume->path($src);

		if (!$this->nameAccepted($source['name'])) {
			return $this->setError(Files_Handler::ERROR_COPY, $errpath, Files_Handler::ERROR_INVALID_NAME);
		}
		...
		if ($source['mime'] == 'directory') {
			...
			if ((!$stat || $stat['mime'] != 'directory') && !$this->_mkdir($destination, $name)) {
				return $this->setError(Files_Handler::ERROR_COPY, $errpath);
			}
	...
	protected function remove($path, $force = false) {
		...
		if (empty($stat)) {
			return $this->setError(Files_Handler::ERROR_RM, $this->_path($path), Files_Handler::ERROR_FILE_NOT_FOUND);
		}

		if (!$force && !empty($stat['locked'])) {
			return $this->setError(Files_Handler::ERROR_LOCKED, $this->_path($path));
		}
		...
		if ($stat['mime'] == 'directory') {
			...
			if (!$this->_rmdir($path)) {
				return $this->setError(Files_Handler::ERROR_RM, $this->_path($path));
			}
