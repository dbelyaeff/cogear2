...
class Widgets_Gear extends Gear {
    ...
    public function hookDispatchAfter($gear) {
        ...
        foreach ($widgets as $widget) {
            $cache_name = 'widgets/'.get_class($widget);
            if(0 === $widget->cache_ttl OR !$code = cache($cache_name)){
                $code = $widget->render();
                cache($cache_name,$code,array('widgets'),$widget->cache_ttl);
            }
            append($widget->render,$code);
        }
//        $widget = widget();
