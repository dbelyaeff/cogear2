<p class="alert alert-info"><?= t('Before start system must check your server for requirements. <br/>Just look at the table below and follow the instructions.') ?></p>
<?
$success = TRUE;
?>
...
<tbody>
    ...
    <tr class="<?= $passed ? 'success' : 'failure' ?>">
        ...
        <td >
            <span class="label label-<?= $passed ? 'success' : 'important' ?>"><?= t($passed ? 'Passed' : 'Error') ?></span>
        </td>
    </tr>
...
<? if ($success): ?>
    <p align="center">
        <a href="<?= l('install/site') ?>" class="btn btn-primary"><?= t('Continue') ?></a>
    </p>
    <?else:?>
